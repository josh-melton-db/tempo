...
from pyspark.sql.types import StructField, StructType, StringType, TimestampType, DateType, DoubleType, IntegerType
...
class TSDFBaseTests(SparkTest):
    ...
    @parameterized.expand([
        ...
        ("simple_date_idx", ["date", "station", "temp"], {"date", "station", "temp"}),
        ("ordinal_double_index", ["*"], {"event_ts_dbl", "symbol", "trade_pr"}),
        ("ordinal_int_index", ["order", "symbol", "trade_pr"], {"order", "symbol", "trade_pr"}),
        ("parsed_ts_idx", ["ts_idx", "trade_pr", "symbol"], {"ts_idx", "trade_pr", "symbol"}),
...
class BaseTransformationTests(SparkTest):
    ...
    def test_withColumnTypeChanged(self, init_tsdf_id, col_to_change, new_type, expected_schema):
        # load TSDF
        tsdf = self.get_test_data(init_tsdf_id).as_tsdf()
        # select column
    ...
    #                                                   StructField('parsed_ts', DateType(), True)]), False)])),
    # ])
    # def test_withColumnTypeChanged_TS(self, init_tsdf_id, col_to_change, new_type, expected_schema):
    #     # load TSDF
    #     tsdf = self.get_test_data(init_tsdf_id).as_tsdf()
    #     # select column
    #     changed_tsdf = tsdf.withColumnTypeChanged(col_to_change, new_type)
